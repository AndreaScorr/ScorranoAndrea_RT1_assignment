...
Future developments could be an increase of the execution speed, and implements the software on a real robot. Maybe the method `R.see()` could be replaced by a camera and using computer vision algorithms to detect the gold boxes and the mehod `R.grab()` could be replaced by a real end effector, fit for purpose. 
